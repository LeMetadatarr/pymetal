...
1. **Endpoint modules return models.** The parsing layer never returns
   ...
   model or a list/iterator of one.
2. **`Client` only does HTTP and cache.** It knows nothing about parsing.
3. **Selectors are data.** Put new XPath, CSS, regex, or URL templates in
   `pymetal/locators.py`. Do not hard-code them in parsers.
...
1. Add the URL template, selectors, and any code tables to `locators.py`.
...
When you add a new fixture, append the target to `_capture.py` and pick a
stable record.

- Use a record that has existed for years, so it is unlikely to disappear.
- Prefer records that exercise the edge case you are testing
  (split-up band, deceased artist, EP, and so on) over headline records.

## Test approach

- **Unit tests** in `test_endpoints_unit.py` and `test_models.py` test
  pure functions, no I/O. New helpers in `_common.py` belong here.
- **Parser tests** in `test_parsers_fixtures.py` use the `FakeClient`
  fixture from `conftest.py` to serve captured HTML. A new endpoint needs
  a new fixture-backed test.
- **HTTP tests** in `test_http.py` swap `Client.session` for a
  recording stub. No fixtures are needed.
...
When you find a parser bug, follow these steps.

1. Capture (or re-capture) a fixture that shows the failure.
2. Write a failing parser test against it.
3. Fix the parser.
4. Confirm the test now passes, then commit both together.

This is how the project was built. Each existing fixture anchors a real
MA quirk: dt/dd label pairs, split band attribution, lineup tab
partitioning, audit-trail blocks.
